...
def play():
    ...
    deck_num = input('How many decks do you want to play with?\n Please choose among 2, 4, 6 and 8\n')
    while deck_num not in ['2','4','6','8']:
        print("That is not a choice!")
        deck_num = input('How many decks do you want to play with?\n Please choose among 2, 4, 6 and 8\n')
    deck_num = int(deck_num)

    #money = input('How much money do you want to start with?\n')
    #while not is_int(money):
        #print("Please input an integer")
        #money = input('How much money do you want to start with?\n')
    #money = int(money)
    num_decks(deck_num,2000)

def num_decks(deck_num, money):
    """
    As the name suggests, this function starts the game with two decks of cards.
    """
    deck = Deck.PlayDeck(deck_num)
    player = Counting_cards.player(money)
    dealer = Player.Player(money, True)
    deck.shuffle()
...
def start_turn(player, dealer, deck):
    ...
    p_hit = player.hit(deck)
    player.counting_cards(p_hit, deck, player)
    d_hit = dealer.hit(deck)
    player.counting_cards(d_hit, deck, player)
    hit = player.hit(deck)
    player.counting_cards(hit, deck, player)
    ...
    if player.sum_hand() == 21:
        ...
        if player.insured and dealer.sum_hand != 21:
...
def pay_insurance(player):
    ...
    print("Do you want to pay $" + str(insurance) + " insurance?(Y/N) ")
    intent = player.is_insurance(player)
...
def player_turn(player, dealer, deck):
    ...
    if not player.split:
        if player.hand[0].face != player.hand[1].face:
            ...
            intent = player.player_decide_split(player, dealer)
            print(intent)
...
def player_first_hand(player, dealer, deck):
    ...
    if player.sum_hand() == 21:
        ...
        print("Do you want to hit, stand or double?")
        print("Press 1 to hit, 2 to stand, 3 to double")
...
def player_second_hand(player, dealer, deck):
    ...
    print("Do you want to hit, stand or double?\nPress 1 to hit, 2 to stand, 3 to double")
    ...
    if player.split:
        if player.is_hit:
            ...
            while answer == '1':
                ...
                hit_card = player.hit(deck)
                player.counting_cards(hit_card, deck, player )
                player.print_hand()
                if player.sum_hand() == 21:
                ...
                if player.busted():
                    ...
                    break
                    print('Press 1 to hit, press any other keys to stand')
                answer = player.player_decide_hit(player,dealer)
            player_second_hand(player, dealer, deck)
        ...
        if player.is_split_hit:
            ...
            while answer == '1':
                ...
                hit_split_card = player.split_hit(deck)
                player.counting_cards(hit_split_card,deck,player)
                player.print_split_hand()
                ...
                if player.busted_split_hand():
                    ...
                    dealer_turn(player, dealer, deck)
                    print('Press 1 to hit, press any other keys to stand')
                answer = player.player_decide_split_hit(player, dealer)
            dealer_turn(player, dealer, deck)
...
def split(player, dealer, deck):
    ...
    hit = player.hit(deck)
    player.counting_cards(hit, deck, player)
    hit_split_hand = player.split_hit(deck)
    player.counting_cards(hit_split_hand,deck, player)
