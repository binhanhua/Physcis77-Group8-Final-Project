...
def start_turn(player, dealer, deck):
    ...
    if player.sum_hand() == 21:
        ...
        if dealer.sum_hand() < 21:
            print("The dealer does not have a natural blackjack. You won!")
            player.take_in(2.5 * player.bet)
        else:
            print("The dealer also has a natural blackjack. It's a tie.")
            player.take_in(player.bet)
    #if the dealer has an ace face up, invoke insurance bet
    elif dealer.hand[1].value == 11:
        print("The dealer has an Ace faced up.")
        pay_insurance(player)
        if dealer.hand[0].value == 10:
            dealer.print_hand()
            print("The dealer does have a natural blackjack.")
            if player.insured:
                print("At least you get some insurance money back.")
                player.take_in(player.bet)
            else:
                print("Unfortunately, you lost, and didn't get any insurance back.")
        else:
            dealer.print_hand()
            print("The hidden card is not a 10-value card. You don't get anything from the insurance.")
            #Normal gameplay, with the interruption of insurance.
            player_turn(player, deck)
            dealer_turn(player, dealer, deck)
    #if the dealer has a natural blackjack but not the players, the round ends.
    elif dealer.sum_hand() == 21:
        dealer.print_hand()
        print("Oh no, the dealer has a natural blackjack. You lost this turn.")
    else:
        #Normal gameplay without naturals or insurance.
        player_turn(player, deck)
        dealer_turn(player, dealer, deck)
